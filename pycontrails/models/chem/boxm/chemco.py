...
# Simple rate coeffs e.g: KRO2NO  = 2.70D-12*np.exp(360/TEMP)
# Complex rate coeffs e.g: K170 = 5.00D-30*((TEMP/298)**(-1.5))*M
# List of all reactions e.g: RC(#) = 5.60D-34*O2*N2*((TEMP/300)**(-2.6))

def chemco(met, chem):

    ts = met["time"].data[0].values
   
    temp = met["air_temperature"].data.isel(time=0)
    #print(temp)
    pressure = met["air_pressure"].data.isel(time=0)
    spec_hum = met["specific_humidity"].data.isel(time=0)
    ...
    RO2 = np.ones_like(temp) # temporary as need to integrate this into main function every timestep
    ...
    # Reaction (1) O = O3 
    chem["RC"].loc[:, :, :, ts, 1] = 5.60e-34*O2*N2*((temp/300)**-2.6)
   
    print(M.loc[0, 0, met["level"].data[8].values].values)
    print(temp.loc[0, 0, met["level"].data[8].values].values)
    print(O2.loc[0, 0, met["level"].data[8].values].values)
    print(N2.loc[0, 0, met["level"].data[8].values].values)

    print(chem["RC"].loc[0, 0, met["level"].data[8].values, ts, 1].values)
