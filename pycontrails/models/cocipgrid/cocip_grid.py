...
import logging
import warnings
from typing import TYPE_CHECKING, Any, Generator, Iterator, NoReturn, TypeVar, overload
...
class CocipGrid(models.Model, cocip_time_handling.CocipTimeHandlingMixin):
    ...
    def _bundle_results(
        ...
        attrs = {
            ...
            **self.source.attrs,  # type: ignore[arg-type]
        }
...
def result_to_metdataset(
    ...
    # NOTE: Calls to `reindex` can occur significant memory overhead!

    # Step 1: Contrail age. Convert from timedelta to float
    contrail_age = np.full(size, 0, dtype=np.float32)
    if result:
    ...
    ef_per_m = np.full(size, 0.0, dtype=dtype)
    if result:
...
def _contrail_grid_variable_attrs() -> dict[str, dict[str, str]]:
    ...
    return {
        "contrail_age": {
            "long_name": "Total age in hours of persistent contrail generated by grid point",
            "units": "hours",
        },
        "ef_per_m": {
        ...
        "sac": {"long_name": "Schmidt-Appleman Criterion at grid point"},
        "persistent": {"long_name": "Contrail initially persistent state"},
        "T_crit_sac": {
            "long_name": "Schmidt-Appleman critical temperature threshold",
            "units": "K",
        },
        "engine_efficiency": {"long_name": "Engine efficiency at grid point"},
        "true_airspeed": {"long_name": "True airspeed at grid point", "units": "m / s"},
        "nvpm_ei_n": {
            "long_name": "Black carbon emissions index number at grid point",
            "units": "kg^{-1}",
        },
        "fuel_flow": {"long_name": "Jet engine fuel flow at grid point", "units": "kg / s"},
        "specific_humidity": {"long_name": "Specific humidity at grid point", "units": "kg / kg"},
    }
...
def _downselect_met(
    ...
    # check params
    longitude_buffer = params.get("met_longitude_buffer", None)
    latitude_buffer = params.get("met_latitude_buffer", None)
    level_buffer = params.get("met_level_buffer", None)
