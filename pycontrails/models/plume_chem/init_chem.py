        ...
        def _init_chem(self) -> xr.Dataset:
                ...
                return xr.Dataset(
                        ...
                        "DJ": (["latitude", "longitude", "level", "time", "photol_coeffs"], 
                                np.ones((len(self.latitude), len(self.longitude), len(self.chem_pressure_levels), len(self.timesteps), len(self.photol_coeffs)))),
                        "RC": (["latitude", "longitude", "level", "time", "therm_coeffs"],
                                np.ones((len(self.latitude), len(self.longitude), len(self.chem_pressure_levels), len(self.timesteps), len(self.therm_coeffs)), dtype=np.float32)),
