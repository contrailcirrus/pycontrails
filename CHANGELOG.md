...
- Add notebook to meteorology documentation on the access and use of experimental ARPEGE forecasts from METEO FRANCE.
...
- Support parsing the `<Function>` elements of `<Waypoint>`s of the ARINC 633 OFP. An additional `"functions"` field is now included in the `Flight` output of `flightplan.parse_ofp_xml`.
- Add a `pickleable` parameter to `GCPCacheStore` controlling whether the underlying GCS client is cached and reused or reconstructed on every access. Set `pickleable=True` to avoid holding a live, unpicklable client on the instance.

### Fixes

- Fix a bug in the MEEM2 nvPM workflow (`Emissions(use_meem=True)`) in which engines with only some smoke numbers were passed to SCOPE11, producing NaN `nvpm_ei_m` and `nvpm_ei_n` arrays. These engines now fall back to constant emissions indices, matching the T4/T2 workflow. Reported in [#426](https://github.com/contrailcirrus/pycontrails/issues/426).
- Distinguish the two causes of the constant nvPM fallback warning in both the T4/T2 and MEEM2 workflows (an engine missing from the EDB, and an engine present with incomplete smoke numbers).
...
- Improve the accuracy of overlaying flight tracks and contrails on Landsat and Sentinel imagery. This includes:
  ...
  - An updated `Sentinel` interface for parsing metadata required for overlay correction (see the new `sentinel_metadata` module for low-level details). This includes a new `colocate_flight` method to overlay flight tracks on Sentinel-2 imagery.
...
- Add new `pycontrails.models.extended_k15` module for modeling volatile particulate matter (vPM) activation in aircraft plumes. This module implements the extended K15 model introduced by [Ponsonby et al.](https://doi.org/10.5194/egusphere-2025-1717).
...
- Refactor `contrail_properties.initial_ice_particle_number` to integrate more smoothly with the vPM activation model. The function's signature has been updated, but its underlying behavior is unchanged. In particular, this has no effect on the `Cocip` model when `vpm_activation` is disabled.
