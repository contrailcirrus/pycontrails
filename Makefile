...
mypy:
	python -c 'import numpy; assert tuple(int(x) for x in numpy.__version__.split(".", 2)) >= (2, 5, 2), f"numpy >= 2.5.2 required, got {numpy.__version__}"'
	mypy pycontrails
...
nb-test: ensure-era5-cached nb-clean-check nb-format-check nb-check-links
	python -m pytest --nbval-lax \
		...
		--ignore=docs/notebooks/ARPEGE.ipynb \
